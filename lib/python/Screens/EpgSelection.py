...
# [GCC 7.4.0]
# Embedded file name: /usr/lib/enigma2/python/Screens/EpgSelection.py
# Compiled at: 2020-01-26 21:58:22
...
class EPGSelection(Screen, HelpableScreen):
    ...
    def __init__(self, session, service=None, zapFunc=None, eventid=None, bouquetChangeCB=None, serviceChangeCB=None, EPGtype=None, StartBouquet=None, StartRef=None, bouquets=None):
        ...
        self['dialogactions'] = HelpableActionMap(self, 'WizardActions', {'back': (
                  self.closeChoiceBoxDialog, _('Close dialog'))}, -1)
        self['dialogactions'].csel = self
        self['dialogactions'].setEnabled(False)
        self['okactions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
                    self.closeScreen, _('Exit EPG')), 
           'OK': (
                self.OK, _('Zap to channel (setup in menu)')), 
           'OKLong': (
                    self.OKLong, _('Zap to channel and close (setup in menu)'))}, -1)
        self['okactions'].csel = self
        self['colouractions'] = HelpableActionMap(self, 'ColorActions', {'red': (
                 self.redButtonPressed, _('IMDB search for current event')), 
           'redlong': (
                     self.redButtonPressedLong, _('Sort EPG List')), 
           'green': (
                   self.greenButtonPressed, _('Add/Remove timer for current event')), 
           'greenlong': (
                       self.greenButtonPressedLong, _('Show Timer List')), 
           'yellow': (
                    self.yellowButtonPressed, _('Search for similar events')), 
           'blue': (
                  self.blueButtonPressed, _('Add a auto timer for current event')), 
           'bluelong': (
                      self.blueButtonPressedLong, _('Show AutoTimer List'))}, -1)
        self['colouractions'].csel = self
        self['recordingactions'] = HelpableActionMap(self, 'InfobarInstantRecord', {'ShortRecord': (
                         self.recButtonPressed, _('Add a record timer for current event')), 
           'LongRecord': (
                        self.recButtonPressedLong, _('Add a zap timer for current event'))}, -1)
        self['recordingactions'].csel = self
        if self.type == EPG_TYPE_SIMILAR:
            self.currentService = service
            self.eventid = eventid
            self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'info': (
                      self.Info, _('Show detailed event info')), 
               'infolong': (
                          self.InfoLong, _('Show single epg for current channel')), 
               'menu': (
                      self.createSetup, _('Setup menu'))}, -1)
            self['epgactions'].csel = self
        elif self.type == EPG_TYPE_SINGLE:
            self.currentService = ServiceReference(service)
            self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextService': (
                             self.nextService, _('Goto next channel')), 
               'prevService': (
                             self.prevService, _('Goto previous channel')), 
               'info': (
                      self.Info, _('Show detailed event info')), 
               'epg': (
                     self.Info, _('Show detailed event info')), 
               'menu': (
                      self.createSetup, _('Setup menu'))}, -1)
            self['epgactions'].csel = self
            self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (
                      self.prevPage, _('Move up a page')), 
               'right': (
                       self.nextPage, _('Move down a page')), 
               'up': (
                    self.moveUp, _('Goto previous channel')), 
               'down': (
                      self.moveDown, _('Goto next channel'))}, -1)
            ...
            if self.type == EPG_TYPE_INFOBAR:
                ...
                self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextBouquet': (
                                 self.nextBouquet, _('Goto next bouquet')), 
                   'prevBouquet': (
                                 self.prevBouquet, _('Goto previous bouquet')), 
                   'nextService': (
                                 self.nextPage, _('Move down a page')), 
                   'prevService': (
                                 self.prevPage, _('Move up a page')), 
                   'input_date_time': (
                                     self.enterDateTime, _('Goto specific data/time')), 
                   'epg': (
                         self.epgButtonPressed, _('Show single epg for current channel')), 
                   'info': (
                          self.Info, _('Show detailed event info')), 
                   'infolong': (
                              self.InfoLong, _('Show single epg for current channel')), 
                ...
                self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (
                   ...
                   'down': (
                          self.moveDown, _('Goto next channel'))}, -1)
                self['epgcursoractions'].csel = self
            self['input_actions'] = HelpableNumberActionMap(self, 'NumberActions', {'0': (
                   self.keyNumberGlobal, _('enter number to jump to channel.')), 
               '1': (
                   self.keyNumberGlobal, _('enter number to jump to channel.')), 
               '2': (
                   self.keyNumberGlobal, _('enter number to jump to channel.')), 
               ...
               '8': (
                   self.keyNumberGlobal, _('enter number to jump to channel.')), 
               '9': (
                   self.keyNumberGlobal, _('enter number to jump to channel.'))}, -1)
            ...
            self['bouquetokactions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (
                        self.BouquetlistHide, _('Close bouquet list.')), 
               'OK': (
                    self.BouquetOK, _('Change to bouquet'))}, -1)
            self['bouquetokactions'].csel = self
            self['bouquetokactions'].setEnabled(False)
            self['bouquetcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (
                      self.moveBouquetPageUp, _('Goto previous event')), 
               'right': (
                       self.moveBouquetPageDown, _('Goto next event')), 
               'up': (
                    self.moveBouquetUp, _('Goto previous channel')), 
               'down': (
                      self.moveBouquetDown, _('Goto next channel'))}, -1)
            self['bouquetcursoractions'].csel = self
            self['bouquetcursoractions'].setEnabled(False)
            self['epgcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (
                      self.leftPressed, _('Goto previous event')), 
               'right': (
                       self.rightPressed, _('Goto next event')), 
               'up': (
                    self.moveUp, _('Goto previous channel')), 
               'down': (
                      self.moveDown, _('Goto next channel'))}, -1)
            self['epgcursoractions'].csel = self
            self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextService': (
                             self.nextService, _('Jump forward 24 hours')), 
               'prevService': (
                             self.prevService, _('Jump back 24 hours')), 
               'nextBouquet': (
                             self.nextBouquet, _('Goto next bouquet')), 
               'prevBouquet': (
                             self.prevBouquet, _('Goto previous bouquet')), 
               ...
               'infolong': (
                          self.InfoLong, _('Show single epg for current channel')), 
               'tv': (
                    self.Bouquetlist, _('Toggle between bouquet/epg lists')), 
               'tvlong': (
                        self.togglePIG, _('Toggle Picture In Graphics')), 
               'menu': (
                      self.createSetup, _('Setup menu'))}, -1)
            self['epgactions'].csel = self
            self['input_actions'] = HelpableNumberActionMap(self, 'NumberActions', {'1': (
                   self.keyNumberGlobal, _('Reduce time scale')), 
               '2': (
                   self.keyNumberGlobal, _('Page up')), 
               '3': (
                   self.keyNumberGlobal, _('Increase time scale')), 
               '4': (
                   self.keyNumberGlobal, _('page left')), 
               '5': (
                   self.keyNumberGlobal, _('Jump to current time')), 
               '6': (
                   self.keyNumberGlobal, _('Page right')), 
               '7': (
                   self.keyNumberGlobal, _('No of items switch (increase or reduced)')), 
               '8': (
                   self.keyNumberGlobal, _('Page down')), 
               '9': (
                   self.keyNumberGlobal, _('Jump to prime time')), 
               '0': (
                   self.keyNumberGlobal, _('Move to home of list'))}, -1)
            self['input_actions'].csel = self
            ...
            self['date'] = Label()
            self.bouquetlist_active = False
            self['bouquetokactions'] = HelpableActionMap(self, 'OkCancelActions', {'OK': (
                    self.BouquetOK, _('Change to bouquet'))}, -1)
            self['bouquetokactions'].csel = self
            self['bouquetokactions'].setEnabled(False)
            self['bouquetcursoractions'] = HelpableActionMap(self, 'DirectionActions', {'left': (
            ...
            self['epgcursoractions'].csel = self
            self['epgactions'] = HelpableActionMap(self, 'EPGSelectActions', {'nextService': (
                             self.nextPage, _('jump to next page or all up (setup in menu)')), 
               'prevService': (
                             self.prevPage, _('jump to previous page or all down (setup in menu)')), 
               'nextBouquet': (
                             self.nextBouquet, _('Goto next bouquet')), 
               'prevBouquet': (
                             self.prevBouquet, _('Goto previous bouquet')), 
               ...
               'epg': (
                     self.epgButtonPressed, _('Show single epg for current channel')), 
               'info': (
                      self.Info, _('Show detailed event info (setup in menu)')), 
               'infolong': (
                          self.InfoLong, _('Show single epg for current channel (setup in menu)')), 
               'tv': (
                    self.Bouquetlist, _('Toggle between bouquet/epg lists')), 
               'tvlong': (
                        self.togglePIG, _('Toggle Picture In Graphics')), 
               'menu': (
                      self.createSetup, _('Setup menu'))}, -1)
            self['input_actions'] = HelpableNumberActionMap(self, 'NumberActions', {'1': (
                   self.keyNumberGlobal, _('goto first channel')), 
               '2': (
                   self.keyNumberGlobal, _('all events up')), 
               '3': (
                   self.keyNumberGlobal, _('goto last channel')), 
               '4': (
                   self.keyNumberGlobal, _('previous channel page')), 
               '0': (
                   self.keyNumberGlobal, _('goto current channel and now')), 
               '6': (
                   self.keyNumberGlobal, _('next channel page')), 
               '7': (
                   self.keyNumberGlobal, _('goto now')), 
               '8': (
                   self.keyNumberGlobal, _('all events down')), 
               '9': (
                   self.keyNumberGlobal, _('Goto Primetime')), 
               '5': (
                   self.keyNumberGlobal, _('Set Basetime'))}, -1)
        if self.type == EPG_TYPE_GRAPH:
    ...
    def OK(self):
        ...
        if not InfoBarInstance.LongButtonPressed:
            if self.zapnumberstarted:
                self.dozumberzap()
            elif self.type == EPG_TYPE_VERTICAL and 'Channel' in config.epgselection.vertical_ok.value:
                self.infoKeyPressed()
    ...
    def OKLong(self):
        ...
        if InfoBarInstance.LongButtonPressed:
            if self.zapnumberstarted:
                self.dozumberzap()
            elif self.type == EPG_TYPE_VERTICAL and 'Channel' in config.epgselection.vertical_oklong.value:
                self.infoKeyPressed()
            elif self.type == EPG_TYPE_GRAPH and config.epgselection.graph_oklong.value == 'Zap' or self.type == EPG_TYPE_ENHANCED and config.epgselection.enhanced_oklong.value == 'Zap' or (self.type == EPG_TYPE_INFOBAR or self.type == EPG_TYPE_INFOBARGRAPH) and config.epgselection.infobar_oklong.value == 'Zap' or self.type == EPG_TYPE_MULTI and config.epgselection.multi_oklong.value == 'Zap' or self.type == EPG_TYPE_VERTICAL and config.epgselection.vertical_oklong.value == 'Zap':
                self.zapTo()
            elif self.type == EPG_TYPE_GRAPH and config.epgselection.graph_oklong.value == 'Zap + Exit' or self.type == EPG_TYPE_ENHANCED and config.epgselection.enhanced_oklong.value == 'Zap + Exit' or (self.type == EPG_TYPE_INFOBAR or self.type == EPG_TYPE_INFOBARGRAPH) and config.epgselection.infobar_oklong.value == 'Zap + Exit' or self.type == EPG_TYPE_MULTI and config.epgselection.multi_oklong.value == 'Zap + Exit' or self.type == EPG_TYPE_VERTICAL and config.epgselection.vertical_oklong.value == 'Zap + Exit':
                self.zap()
        ...
        if self.type == EPG_TYPE_SINGLE:
            ...
            if self.session.nav.getCurrentlyPlayingServiceOrGroup() and self.StartRef and self.session.nav.getCurrentlyPlayingServiceOrGroup().toString() != self.StartRef.toString():
                if self.zapFunc and self.StartRef and self.StartBouquet:
                    if self.type == EPG_TYPE_GRAPH and config.epgselection.graph_preview_mode.value or self.type == EPG_TYPE_MULTI and config.epgselection.multi_preview_mode.value or self.type in (EPG_TYPE_INFOBAR, EPG_TYPE_INFOBARGRAPH) and config.epgselection.infobar_preview_mode.value in ('1',
                                                                                                                                                                                                                                                                                                     '2') or self.type == EPG_TYPE_ENHANCED and config.epgselection.enhanced_preview_mode.value or self.type == EPG_TYPE_VERTICAL and config.epgselection.vertical_preview_mode.value:
    ...
    def setMinus24h(self, force=False, daypart=1):
        ...
        if ev_begin is not None:
            if ev_begin - oneDay < now:
                self.ask_time = -1
            elif self[('list' + str(self.activeList))].getCurrentIndex() and not force and self.findMinEventTime(ev_begin - oneDay):
                self.lastEventTime = (
                 ev_begin - oneDay, ev_end - oneDay)
                self.gotoLasttime()
                return
            pt = 0
    ...
    def gotoPrimetime(self):
        ...
        if ev_begin is None:
            ...
            if rPM or rPT or rPP:
                if onlyPT or idx or not (primetime >= ev_begin and primetime < ev_end):
                    if rPT:
